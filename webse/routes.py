...
from webse import app, db, bcrypt, testdf, DB_VAR
...
from webse.forms import ModStatisticsForm, AnnouncementForm
...
@app.route('/')
def home():
    announcement_test=testdf("announcement",DB_VAR).test_bin
    chat_test=testdf("chat",DB_VAR).test_bin
    page = request.args.get('page', 1, type=int)
    
    if announcement_test:
    ...
    if chat_test:
        if page<=min_pages:
            ...
            chats = Chat.query.order_by(Chat.date_posted.desc()).paginate(page=max_pages_chats, per_page=2)
        else:
            chats = Chat.query.order_by(Chat.date_posted.desc()).paginate(page=page, per_page=2)
    else:
        chats=None
...
def chat_web():
    ...
    if chat_form.validate_on_submit() and chat_test:
        input_chat_module=chat_form.chat_module.data
        input_chat_group=chat_form.chat_group.data
        ...
        return render_template('chat_web.html', title='Chat Web',
                           form=chat_form, legend='Chat Web', chats=chats)
...
@login_required
def statistics():
    test_moduls=testdf("moduls",DB_VAR).test_bin
    ...
    if mod_form.validate_on_submit() and test_moduls:
        ...
        return render_template('statistics.html', mod_form=mod_form)
